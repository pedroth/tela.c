 ...
 * Features interactive orbit camera controls via mouse and WASD camera
 * movement.
 */
...
static void on_frame(f32 dt, f32 time, void* ctx) {
  ...
  app->camera->position =
      add_vec3(app->camera->position, scale_vec3(world_speed, dt));
  app->camera->look_at =
      add_vec3(app->camera->look_at, scale_vec3(world_speed, dt));

  // g_directional_light->direction = normalize_vec3(vec3(cosf(time * 0.1f),
  // 0.0f, sinf(time * 0.1f))); Render
...
static void on_mouse_move(Window* window, i32 x, i32 y, void* ctx) {
  ...
  Vec2 orient = get_camera_orient(app->camera);
  f32 theta_delta = -2.0f * PI * (delta.x / WIDTH);
  f32 phi_delta = -2.0f * PI * (delta.y / HEIGHT);

  set_orient_camera(app->camera, orient.x + theta_delta, orient.y + phi_delta);
...
static void on_key_down(Window* window, u32 keycode, void* ctx) {
  ...
  if (keycode == SDLK_w)
    app->cam_speed = vec3(0, 0, magnitude);
  if (keycode == SDLK_s)
    app->cam_speed = vec3(0, 0, -magnitude);
  Tela* exposed = app->tela;
  exposed->iterations = 1;
...
// Vertex transform: v => Vec3(v.x, -v.z, v.y)
Vec3 swizzle_vertex(Vec3 v, void* ctx) {
  return vec3(v.x, -v.z, v.y);
}

int main(void) {
  g_background = io_read_image("assets/sky.jpg");
  Vec3 light_dir = vec3(1.0f, 0.0f, 0.5f);
  light_dir = normalize_vec3(light_dir);
  g_directional_light =
      &(DirectionalLightParams){ .direction = light_dir, .sharpness = 200.0f };
  ...
  // Load mesh
  String obj = io_read_file("./assets/minecraft.obj");
  Mesh mesh = read_obj_mesh(obj, "minecraft");
  ...
  map_vertices_mesh(&mesh, swizzle_vertex, NULL);
