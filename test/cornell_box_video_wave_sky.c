...
// gcc -O3 -fopenmp -o app test/cornell_box_video_wave_sky.c -lSDL2 -lm
...
static void on_frame(f32 dt, f32 time, void* ctx) {
  ...
  f32 theta = (f32)(PI / 40.0f) * time;
  set_orbit_camera(app->camera, orbit.x, theta, orbit.z);
...
int main(void) {
  ...
  *app.mat_wave_dielectric = build_dielectric_material(1.45f);
  ...
  u32 t0 = get_time_ms();
  loop_to_video(animation, "cornell_box_video_wave_sky.mp4", loop_params);
  u32 t1 = get_time_ms();
