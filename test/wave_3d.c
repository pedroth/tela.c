  ...
  for (u32 i = 0; i < GRID_SIZE; i++) {
    for (u32 j = 0; j < GRID_SIZE; j++) {
      for (u32 k = 0; k < GRID_SIZE; k++) {
        ...
        f32 bump1 = AMPLITUDE * expf(-SPREAD * ((x - 0.25f) * (x - 0.25f) + y * y + z * z));
        f32 bump2 = AMPLITUDE * expf(-SPREAD * ((x + 0.25f) * (x + 0.25f) + y * y + z * z));
        f32 bump3 = AMPLITUDE * expf(-SPREAD * (x * x + (y - 0.25f) * (y - 0.25f) + z * z));
        f32 bump4 = AMPLITUDE * expf(-SPREAD * (x * x + y * y + (z + 0.25f) * (z + 0.25f)));
...
static void on_frame(f32 dt, f32 time, void* ctx) {
  ...
  for (u32 i = 0; i < GRID_SIZE; i++) {
    for (u32 j = 0; j < GRID_SIZE; j++) {
      for (u32 k = 0; k < GRID_SIZE; k++) {
        ...
        f32 acceleration = WAVE_SPEED * laplacian - FRICTION * g_velocity[i][j][k];
        ...
        if (g_density[i][j][k] > max_h) max_h = g_density[i][j][k];
        if (g_density[i][j][k] < min_h) min_h = g_density[i][j][k];
  ...
  set_window_title(app->window,
    format_string("Volumetric | FPS: %.1f", 1.0f / dt));
...
static void paint_density(App* app, f32 mx, f32 my) {
  ...
  if (hit.hit) {
    ...
    Vec3 normal = ray_from_tela_camera(app->camera, app->tela, WIDTH / 2, HEIGHT / 2).dir;
...
static void on_mouse_up(Window* window, i32 x, i32 y, u32 button, void* ctx) {
  if (button == 1) g_left_mouse_down = false;
  if (button == 3) g_right_mouse_down = false;
}
...
static void on_mouse_scroll(Window* window, i32 delta_y, void* ctx) {
  ...
  if (new_radius < 0.1f) new_radius = 0.1f;
