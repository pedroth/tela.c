...
bool clip_line(Vec2 p0, Vec2 p1, AABB_2D box, Line_2D *clipped_line) {
  ...
  if (p0_inside && !p1_inside) {
    ...
    if (intersection.length == 0)
      return false;
    clipped_line->start = p0;
    clipped_line->end = intersection.points[0];
    return true;
  }
  ...
  if (!p0_inside && p1_inside) {
    ...
    if (intersection.length == 0)
      return false;
    clipped_line->start = intersection.points[0];
    clipped_line->end = p1;
    return true;
  }
  ...
  if (intersection.length < 2)
    return false;
  clipped_line->start = intersection.points[0];
  clipped_line->end = intersection.points[1];
  return true;
}
...
static inline Tela *draw_line_tela(Tela *tela, const Line_2D *line,
  ...
  clipped_line.context = line->context;
  Vec2 pi = clipped_line.start;
